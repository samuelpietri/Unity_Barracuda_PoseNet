...
public class PoseEstimator : MonoBehaviour
{
    ...
    // PoseNet model outputs
    // Heatmaps = low resolution version of the input image where each pixel 
    // contains a value indicating how confident the model is that a given point is in that spot
    // There is a heatmap for each keypoint predficted by the model

    // Offset = used to refile the rough locations form the heatmaps. 
    // There are two offset for each keypoint (X, Y axes). These values are
    // added to the coordinates estimated byu the heatmaps to scale the coordinate back up to the input resolution

    // displacementFWDLayer and displacementBWDLayer = needed for multi-pose estimation , used to identify key points
    // that belong to the same body in an image. 

    // The name of the outputs are different for the MObileNet and ResNet models
    ...
    private struct Engine
    {
        ...
        // The name for the heatmap layer int he model asset
        ...
        // The name for the Sigmoid layer that return  the heatmnap predictions
    ...
    [Tooltip("The ResNet50 model asset file to use when performing nference")]
    public NNModel resnetModelAsset;

    // Target dimensions fo model input
    ...
    [Tooltip("The requested video frame rate")]
    ...
    private void InitializeBarracuda()
    {
        ...
        // Validate iuf backend is supported, otherwise use fallback type
        workerType = WorkerFactory.ValidateType(workerType);
        // Create a worker that will execute the model withe the selected backend
    ...
    private void InitializeVideoScreen(int width, int height, bool mirrorScreen){
        ...
        //Adjust the VideoScreen position fo rthe new videoTexture
    ...
    private void InitializeCamera()
    {
        ...
        //Adjust the camera position to account for udates to the VideoScreen
    ...
    void Start()
    {
       ...
       // we need to initialize the videoTexture with the new dimensions and the 
       // ARGBHalf HDR texture format because we can store color values outside
       // the standard Uniti range of [0, 1].
       // MobileNet version expects values to be in the range [-1, 1]
       // ResNet50 expect values in the range [0, 255]
        ...
        // Adjust the input diensnions to maintain the source aspect ratio
    ...
    void Update()
    {
        ...
        // Prevent the input dimensions form going too low for the model
    ...
    {
        {
            // Release the resoruces allocated for the ifnerence engine
            engine.worker.Dispose();
        }
    }
}
